 ...
 *
 */
public class ScalaAdapter implements ILanguageAdapter
{
    private Logger log = LogManager.getLogger("ScalaAdapter");
    ...
    public void setProxy(Field target, Class<?> proxyTarget, Object proxy) throws IllegalArgumentException, IllegalAccessException, NoSuchFieldException, SecurityException
    {
        ...
        catch (ClassNotFoundException e)
        {
            ...
            log.info("An error occurred trying to load a proxy into {}.{}. Did you declare your mod as 'class' instead of 'object'?", proxyTarget.getSimpleName(), target.getName());
        ...
        {
            log.error("An error occurred trying to load a proxy into {}.{}", proxyTarget.getSimpleName(), target.getName());
            throw new LoaderException(e);
        }

        // If we come here we could not find a setter for this proxy.
        log.error("Failed loading proxy into {}.{}, could not find setter function. Did you declare the field with 'val' instead of 'var'?", proxyTarget.getSimpleName(), target.getName());
            ...
            for (Field target : proxyTarget.getDeclaredFields())
            {
                ...
                if (target.getAnnotation(SidedProxy.class) != null)
                {
                    ...
                    try
                    {
                        ...
                        {
                            log.error("Attempted to load a proxy type %s into %s.%s, but the types don't match", targetType, proxyTarget.getSimpleName(), target.getName());
                        ...
                        log.error("{}\nAn error occurred trying to load a proxy into {}.{}", e, proxyTarget.getSimpleName(), target.getName());
                        throw new LoaderException(e);
                    }
                }
            }
